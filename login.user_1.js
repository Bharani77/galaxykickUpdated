...
(function() {
  ...
  // --- Load External Values (Called before execution) ---
// Reads parameters set by Puppeteer via GM_setValue
async function loadExternalValues() {
  ...
  try {
      ...
      // FIX: Properly handle array of rival names
  ...
  window.executeTampermonkeyLogic = async (messageType) => {
      log(`[UserScript] executeTampermonkeyLogic called by Puppeteer. Trigger Type: ${messageType}`);
      ...
      try {
          // Load latest rival name and timing params from GM storage
          const loadedOK = await loadExternalValues();
